...
ax.plot(rau, sig_A12_g/zeta, '-', color='gray', linewidth=3)

sigd10, gam, rc = 0.39, 0.75, 60.
sig_A12_d = sigd10 * (rau/10.)**(-gam) 
sig_A12_d[rau > rc] = 1e-10
ax.plot(rau, sig_A12_d, '-m', linewidth=3)
...
ax.plot(rau, sig_F18_g, '-', color='gray', linewidth=3)


### Zhang 2017 13C18O
sig_Z17_g = 13. * (rau/20.5)**(-0.9)
zone = (rau >= 5.) & (rau <= 21.)
ax.plot(rau[zone], sig_Z17_g[zone], '-', color='gray', linewidth=3)
...
sig_vB17_d = dint(rau)
ax.plot(rau, sig_vB17_g, '-', color='gray', linewidth=3)
ax.plot(rau, sig_vB17_d, '-m', linewidth=3)
...
ax.plot(rau, sig_H16_d, '-m', linewidth=3)
